...
* Calculation result after the third transformation is ``r factor_to_numeric * solution_df$vec_sample``. In this transformation, R converted factor vector to numeric term in this manner: Factor_Level 1 = 1, Factor_Level 2 = 2 etc. The factor name are arragned alphabetically (increasing order, see `?factor` for detail). In this sample, level  `FALSE` equal to 1 and level `TRUE` equal to 2.  In this example, ``r (solution_df$vec_sample[solution_df$vec_sample<=0])[1]``, position ``r which(solution_df$vec_sample<=0)[1]``  in the `vec_sample` is ``r (factor_to_numeric * solution_df$vec_sample)[which(solution_df$vec_sample<=0)[1]]`` (`FALSE = 1`) ``r (solution_df$vec_sample[solution_df$vec_sample>0])[1]``, position ``r which(solution_df$vec_sample>0)[1]`` in the `vec_sample` is ``r (factor_to_numeric * solution_df$vec_sample)[which(solution_df$vec_sample>0)[1]]``(`TRUE = 2`).  
